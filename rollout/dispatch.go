...
// ReconcileStatuses reapplies rollout aggregation after state recovery.
func (e *Engine) ReconcileStatuses(ctx context.Context) error {
	e.mu.Lock()
	defer e.mu.Unlock()
	for id := range e.state.Rollouts {
		if err := e.aggregateLocked(ctx, id); err != nil {
			return err
		}
	}
	return nil
