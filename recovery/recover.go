...
func Recover(ctx context.Context, store persistence.Store, now rollout.Clock) (*rollout.Engine, Report, error) {
	...
	// Reconcile status when recovery found work waiting for dispatch.
	if report.PendingTargets > 0 {
		if err := engine.ReconcileStatuses(ctx); err != nil {
			return nil, Report{}, fmt.Errorf("reconcile recovered pending targets: %w", err)
		}
	}
